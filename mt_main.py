...
# This script contains the training, validation, and test of the Multimodel Multi-Task (MT) model using cross-validation sets for all available annotators 
...
split_sets = {}
split_sets['train_mv'] = ["BMI","CHI","FNE","GLA","LOR"]
split_sets['test_mv'] = ["CRA","DEP"]
# datasets for train,val and test with all necessary preprocessing 
select_viewer = 7  # select viewer number between 0 and 7.
# load data and divide in to train,val and test for single set of cross-validation
visual_train, text_train, audio_train, train_label, visual_test, text_test, audio_test, test_label = utils.create_train_val_test_datasets_mt_joint1(split_sets,all_viewers,select_viewer)
# text preprocessing
# data cleaning
...
# inputs and labels 
x_conv = tf.placeholder(tf.int32, shape=([None,18]))
x_cmb = tf.placeholder(tf.float32, shape=([None,14512]))
...
conv_features = conv_net(x_conv, VOCAB_LEN, EMBED_SIZE, biases)
...
optimizer_5 = tf.train.GradientDescentOptimizer(learning_rate=learning_rate)
...
with tf.Session() as sess:
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(0,len(idx_list),batch_size):
            batch_index = idx_list[i:i+batch_size]
        ...
        for batch_no in range(len(batch_info)):
            ...
            for idx in range(len(single_batch)):
                ...
                x_audio.append(audio_train[single_batch[idx]])
            ...
            # combine modalities
            cmb_txt_vis_features = np.concatenate((text_features, x_visual), axis=1)
            cmb_features = np.concatenate((cmb_txt_vis_features, x_audio), axis=1)   
            # shared fully-connected layers  
        ...
        training_weight = neg_count/pos_count
        for k in range(0,len(test_label),batch_size):
            if k == int(len(test_label)/batch_size)*batch_size:
                x_visual = visual_test[k:]
                x_text = text_test[k:]
                x_audio = audio_test[k:]
                input_y = test_label[k:]
                ground_truth.append(input_y)
                input_y = np.reshape(input_y,[len(input_y),1])
            else:
                x_visual = visual_test[k:k+batch_size]
                x_text = text_test[k:k+batch_size]
                x_audio = audio_test[k:k+batch_size]
                input_y = test_label[k:k+batch_size]
                ground_truth.append(input_y)
                input_y = np.reshape(input_y,[len(input_y),1])
        ...
        acc, true_p, true_n, false_p, false_n = utils.md_performance(gt_binary,pred_binary)
